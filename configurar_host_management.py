...
def create_sw3_flows(switch_dpid: str, host_ip: str, host_port: str) -> List[Dict]:
    ...
    flows = [
        ...
        # TCP: desde host hacia Management (192.168.200.13)
        {
            "switch": switch_dpid,
        ...
        # TCP: desde Management (192.168.200.13) hacia host
        {
            "switch": switch_dpid,
        ...
        # ICMP: desde host hacia Management (192.168.200.13)
        {
            "switch": switch_dpid,
            "name": f"sw3_icmp_{port_normalized}_to_1_{ip_normalized}",
            "table": "1",
            "priority": "200",
            "active": "true",
            "in_port": host_port,
        ...
        # ICMP: desde Management (192.168.200.13) hacia host
        {
            "switch": switch_dpid,
            "name": f"sw3_icmp_1_to_{port_normalized}_{ip_normalized}",
            "table": "1",
            "priority": "200",
            "active": "true",
            "in_port": "1",
...
def create_sw4_flows(switch_dpid: str, host_ip: str) -> List[Dict]:
    ...
    flows = [
        ...
        {
            "switch": switch_dpid,
            "name": f"sw4_icmp_5_to_1_{ip_normalized}",
            "table": "1",
            "priority": "200",
            "active": "true",
            "in_port": "5",
        ...
        {
            "switch": switch_dpid,
            "name": f"sw4_icmp_1_to_5_{ip_normalized}",
            "table": "1",
            "priority": "200",
            "active": "true",
            "in_port": "1",
...
def create_sw1_flows(switch_dpid: str, host_ip: str) -> List[Dict]:
    ...
    flows = [
        ...
        {
            "switch": switch_dpid,
            "name": f"sw1_icmp_3_to_4_{ip_normalized}",
            "table": "1",
            "priority": "200",
            "active": "true",
            "in_port": "3",
        ...
        {
            "switch": switch_dpid,
            "name": f"sw1_icmp_4_to_3_{ip_normalized}",
            "table": "1",
            "priority": "200",
            "active": "true",
            "in_port": "4",
...
def main():
    ...
    print(f"[OK] Controlador conectado. Switches encontrados: {len(switches)}")
    for switch in switches:
    ...
    if not sw1_dpid or not sw3_dpid or not sw4_dpid:
        print("\n[WARNING] No se pudieron identificar automáticamente los switches por IP.")
        ...
        if not sw1_dpid:
            try:
                ...
                if 0 <= idx < len(switches):
                    ...
                    print(f"[OK] sw1 seleccionado: {sw1_dpid}")
                else:
        ...
        if not sw3_dpid:
            try:
                ...
                if 0 <= idx < len(switches):
                    ...
                    print(f"[OK] sw3 seleccionado: {sw3_dpid}")
                else:
                    print("ERROR: Selección inválida")
                    sys.exit(1)
            except (ValueError, KeyboardInterrupt):
        ...
        if not sw4_dpid:
            try:
                ...
                if 0 <= idx < len(switches):
                    ...
                    print(f"[OK] sw4 seleccionado: {sw4_dpid}")
                else:
                    print("ERROR: Selección inválida")
                    sys.exit(1)
            except (ValueError, KeyboardInterrupt):
        ...
        print(f"[OK] sw1 encontrado por IP {switch_ips['sw1']}: {sw1_dpid}")
        print(f"[OK] sw3 encontrado por IP {switch_ips['sw3']}: {sw3_dpid}")
        print(f"[OK] sw4 encontrado por IP {switch_ips['sw4']}: {sw4_dpid}")
    
    # Obtener información del host (desde parámetros o input)
    host_ip = None
    host_port = None
    ...
    if len(sys.argv) >= 3:
        ...
        print(f"\n[3] Configuración del host cliente (desde parámetros)...")
        print(f"[OK] Host configurado: {host_ip} en SW3 puerto {host_port}")
    else:
    ...
    print(f"[OK] {len(flows_sw1)} flujos para sw1")
    print(f"[OK] {len(flows_sw3)} flujos para sw3")
    print(f"[OK] {len(flows_sw4)} flujos para sw4")
    ...
    if success_sw1 == len(flows_sw1) and success_sw3 == len(flows_sw3) and success_sw4 == len(flows_sw4):
        print("\n[OK] Todos los flujos se instalaron correctamente!")
        return 0
    else:
        print("\n[WARNING] Algunos flujos no se pudieron instalar. Revisa los errores arriba.")
        return 1
